...
| OmO 没识别到 OpenSpec skills | 项目根目录运行 `openspec update` |
...
| tdd-driven schema 找不到 | [openspec-tdd-setup.md](./openspec-tdd-setup.md) |
| `/harness-scan` 产出太长 | 不变量留根文件，详细内容下沉 docs/ |
| Agent 忽略 AGENTS.md 规则 | `/harness-gate` 把规则变成 lint/test |
| 存量项目开 strict 后构建全红 | `/harness-gate` 用冻结-棘轮法 |
| timely-doc-garden 扫描不到项目 | 编辑 `projects.yaml` 或 `/harness-doc-garden` |
