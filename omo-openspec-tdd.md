...
|---|---|---|---|
| **盘点**（项目知识文档化） | OmO 的 `AGENTS.md` 系统 | 根文件 ≤100 行 + `docs/` | `/init-deep` |
| **分层**（根文件 vs docs/） | OmO 的 hierarchical AGENTS.md | `AGENTS.md`（根）+ 子目录 `AGENTS.md` | `/init-deep` |
| **设卡**（自动化检查） | TDD 的质量闸门 + CI | lint / type-check / test gates | Prometheus 面谈 → Atlas 执行 |
| **维护**（文档 gardening） | doc-garden Skill | 定期 doc-gardening + 规则回顾 | `doc-garden` Skill + cron/launchd |
...
| Agent 权限（OpenSpec spec-only vs OmO 全能） | 阶段分离：规划阶段用 `openspec-plan` Agent（锁死权限），执行阶段切 Sisyphus |
...
# 4. opencode-plugin-openspec（OpenSpec ↔ OmO 桥接插件）
...
确保 `opencode.json`（或 `.opencode/opencode.json`）中同时包含：

```jsonc
{
  "plugin": [
    "opencode-plugin-openspec"   // OpenSpec 规划 Agent
  ],
  // OmO 的其他配置...
}
```

`opencode-plugin-openspec` 会在检测到 `openspec/` 目录时自动注入 `openspec-plan` Agent（只读代码、只写 spec 文件的权限模型）。非 OpenSpec 项目零开销。
...
**盘五件事**（init-deep 自动盘 + 人工补充）：

| 维度 | init-deep 自动盘 | 需要人工补充 |
|------|-----------------|-------------|
...
| 安全红线 | ⚠️ 只查文件名和位置 | 凭证位置（只标 `文件名:行号`，绝不复制值）、secret 管理策略 |
| 隐性知识 | ❌ 无法自动发现 | 历史坑、"大家都知道但没写下来"的事 |

**`/init-deep` 跑完后的必做 checklist**（init-deep 做不到、必须人工补的三件事）：

- [ ] **安全红线**：检查根 AGENTS.md 中是否缺少安全红线节；如果有 `.env`、`*secret*`、`*credential*` 等文件，确认只标注了 `文件名:行号` 而未复制值
- [ ] **隐性知识**：补充 init-deep 无法发现的团队约定（如"这个模块是历史遗留，新代码别碰"、"这个 API 有隐藏的 rate limit"），标记格式 `[TODO: confirm with team]`
- [ ] **多仓库漂移**（如有多个项目）：对比各仓库的 AGENTS.md，把不一致的地方（框架版本、包前缀、lint 配置）标注为漂移警告
...
### 4.4 Day 2-3：设卡（Prometheus 面谈模式）
...
Prometheus 自动执行：
  ┌─ Explore 1: 扫描 CI config (.github/workflows/, Makefile, docker-compose)
  ├─ Explore 2: 扫描 lint/test 配置 (eslint, vitest, pytest, Cargo test)
  └─ Explore 3: 扫描 pre-commit hooks (.husky/, .pre-commit-config.yaml)

Prometheus: "我发现项目已有以下检查：
  - ESLint (JavaScript)
  - Vitest (测试)
  缺少：
  - 架构分层检查（src/data 不应 import src/api）
  - 类型检查（TypeScript strict mode 未完全开启）
  - 覆盖率阈值
  你想加哪些？"

你: 确认要加的检查

Prometheus: "Agent 最常犯什么错？你希望先拦什么？"
你: 回答高频违规

Prometheus → 生成计划 → Momus 审核 → Atlas 执行安装配置
```
...
### 4.5 持续：维护（doc-garden Skill）

让文档活着。过期的记忆比没有记忆更危险——Agent 读到错误的文档，产出的就是幻觉。

**自动化方案**：`doc-garden` Skill — 定时扫描所有注册项目，自动检测并修复文档过时问题。

#### 安装

Skill 位置：`/Users/chenchaowen/Desktop/Project/dev-tooling/.agents/.skills/doc-garden/`

确保 `opencode` 能发现该 skill（在 `opencode.json` 中配置 skill 路径，或将 skill 目录 symlink 到 `~/.agents/skills/`）。
...
crontab -e
# 添加：
0 9 * * 1 /bin/bash /Users/chenchaowen/Desktop/Project/dev-tooling/.agents/.skills/doc-garden/scripts/run-scheduled.sh >> /tmp/doc-garden-cron.log 2>&1
```

**方式 B：launchd（推荐，macOS 原生）**：
```bash
# 创建 plist（参考 run-scheduled.sh 内的 launchd 示例）
launchctl load ~/Library/LaunchAgents/com.opencode.doc-garden.plist
```

**方式 C：手动触发**：
```bash
# 扫描所有注册项目
bash /Users/chenchaowen/Desktop/Project/dev-tooling/.agents/.skills/doc-garden/scripts/run-scheduled.sh

# 扫描单个项目
bash /Users/chenchaowen/Desktop/Project/dev-tooling/.agents/.skills/doc-garden/scripts/run-scheduled.sh --project /path/to/project

# 在项目内直接调用 opencode
opencode run --command doc-garden "Run doc-garden check"
```
...
run-scheduled.sh → 读取 projects.yaml → 逐项目执行
    ↓
opencode run --command doc-garden
    ↓
...
| 维护维度 | doc-garden（自动化） | OpenSpec archive（功能开发时） |
|---------|---------------------|-------------------------------|
        ...
        "Write|Edit|MultiEdit": [
          {
            "command": [
              "bash",
              "/Users/chenchaowen/Desktop/Project/dev-tooling/.agents/.skills/doc-garden/scripts/validate-refs.sh"
            ],
            "environment": { "TRIGGER": "file_edited" }
          }
        ]
      },
      // Session 结束后触发 — 轻量 doc-garden 扫描
      "session_completed": [
        {
          "command": [
            "bash",
            "/Users/chenchaowen/Desktop/Project/dev-tooling/.agents/.skills/doc-garden/scripts/run-scheduled.sh",
            "--project", "."
          ],
...
| `session_completed` | 整个对话结束时 | 全量 doc-garden 扫描（只检查本次变更涉及的文档） |
...
> **前提**：已完成 Harness Init（第零层），项目已有 `AGENTS.md` + `docs/` + 质量闸门。如果是全新项目，先跑一遍 `/init-deep` + `oinit`。

### 5.1 Phase 1：初始化功能

```bash
# 新项目（首次接入）
/init-deep                        # Harness Init: 盘点+分层
...
### 5.2 Phase 2：规划（openspec-plan Agent）

在 OmO 中切换到 `openspec-plan` Agent（权限锁死，只能写 spec 文件）。
...
| **规划**（propose） | `openspec-plan` | 只读代码，只写 spec 文件 | 防止 Agent 在规划阶段就开始写代码 |
| **执行**（apply） | `Sisyphus` | 全权限 | 需要写代码、跑测试、改文件 |
| **验证**（verify） | `Sisyphus` 或 `openspec-plan` | 读权限为主 | 偏审查性质 |
| **归档**（archive） | `Sisyphus` | 需要移动文件 | 涉及文件系统操作 |

**关键**：在规划阶段使用 `openspec-plan` 的权限锁，杜绝"规划着规划着就开始写代码"的 AI 通病。
...
```bash
# Day 1: 盘点 + 分层
/init-deep                        # OmO 生成 hierarchical AGENTS.md
# → 人工补充：安全红线、隐性知识、[TODO: confirm with team] 项

# Day 2-3: 设卡
/prometheus
# → 面谈：探索现有检查 → 确认要加的 → Atlas 执行安装
# → 棕地项目用冻结-棘轮法，不直接开 strict

# Day 3+: 注册到 doc-garden 定时扫描
# 编辑 projects.yaml，添加项目路径
```
...
| Agent 在规划阶段写代码 | 没有使用 `openspec-plan` Agent | 确认 `opencode-plugin-openspec` 已安装，切换到 `openspec-plan` |
...
| `opencode-plugin-openspec` 不生效 | 未在 `opencode.json` 中注册 | 添加到 `plugin` 数组 |
| tdd-driven schema 找不到 | 全局 schema 未安装 | 参考 [openspec-tdd-setup.md](./openspec-tdd-setup.md) 安装 |
| `/init-deep` 生成的 AGENTS.md 太长 | 存量项目信息量大 | 手动瘦身：不变量留根文件，详细内容下沉 docs/ |
| Agent 忽略 AGENTS.md 中的规则 | 规则只是文字，不可执行 | Step 3 设卡：把关键规则变成 lint / test |
| 存量项目开 strict 后构建全红 | 一次开太严了 | 用冻结-棘轮法：先宽松，逐步收紧 |
| doc-garden 扫描不到项目 | 未在 `projects.yaml` 中注册 | 编辑 skill 目录下的 `projects.yaml` 添加项目路径 |
| `opencode run --command doc-garden` 不触发 skill | skill 目录不在 discovery path | symlink 到 `~/.agents/skills/` 或在 `opencode.json` 配置 skill 路径 |
...
| OmO 插件配置 | `~/.config/opencode/opencode.json` | 注册 `opencode-plugin-openspec` |

### 全局工具 — doc-garden Skill
...
| 报告模板 | `dev-tooling/.agents/.skills/doc-garden/templates/report.md` | 输出报告格式 |
...
| doc-garden 报告 | `.sisyphus/doc-garden-report.md` | 每次 doc-garden 运行的结果报告 |
...
- [opencode-plugin-openspec](https://github.com/Octane0411/opencode-plugin-openspec) — 桥接插件
...
- [doc-garden skill](./.agents/.skills/doc-garden/SKILL.md) — 文档-代码一致性自动检查 Skill
